    ...
    'columns' => [
            ...
            'buttons' => [
                'reject' => function ($url, $model, $id) {
                    ...
                    if ($model->status != \common\models\Offer::STATUS_CANCEL && $model->status != \common\models\Offer::STATUS_OK && $model->status != \common\models\Offer::STATUS_REJECT) {

                        if ($model->buyer_id != $model->user_id && $model->seller_id != $model->user_id) {

                            if ($model->author_id != Yii::$app->user->identity->id && $model->status != \common\models\Offer::STATUS_PENDING && $model->user_id != Yii::$app->user->identity->id) {
                            ...
                            if ($model->user_id == Yii::$app->user->identity->id && $model->status == \common\models\Offer::STATUS_PENDING) {
                                return Html::a(' <button type="button" class="btn btn-danger dropdown-toggle"  aria-haspopup="true" aria-expanded="false"><span class="glyphicon glyphicon-remove"></span></button>', $url, ['title' => Yii::t('app', 'Reject')]);
                        ...
                        if (($model->buyer_id == $model->user_id || $model->seller_id == $model->user_id)) {
                            ...
                            if ($model->status != \common\models\Offer::STATUS_PENDING && $model->author_id != Yii::$app->user->identity->id) {
                                return Html::a(' <button type="button" class="btn btn-danger dropdown-toggle"  aria-haspopup="true" aria-expanded="false"><span class="glyphicon glyphicon-remove"></span></button>', $url, ['title' => Yii::t('app', 'Reject')]);
                            }
                        }
                    }
                ...
                'ok' => function ($url, $model, $id) {
                    if ($model->status != \common\models\Offer::STATUS_CANCEL && $model->status != \common\models\Offer::STATUS_OK && $model->status != \common\models\Offer::STATUS_REJECT) {
                        ...
                        if ($model->buyer_id != Yii::$app->user->identity->id && $model->seller_id != Yii::$app->user->identity->id) {

                            if ($model->author_id != Yii::$app->user->identity->id && $model->status == \common\models\Offer::STATUS_PENDING) {
                                return Html::a(' <button type="button" class="btn btn-success dropdown-toggle"  aria-haspopup="true" aria-expanded="false"><span class="glyphicon glyphicon glyphicon-ok"></span></button>', $url, ['title' => Yii::t('app', 'Ok')]);
                            }
                        }
                ...
                'pending' => function ($url, $model, $id) {
                    if ($model->status != \common\models\Offer::STATUS_CANCEL && $model->status != \common\models\Offer::STATUS_OK && $model->status != \common\models\Offer::STATUS_PENDING && $model->user_id != Yii::$app->user->identity->id && $model->status != \common\models\Offer::STATUS_REJECT) {
                        if (($model->buyer_id != $model->user_id && $model->seller_id != $model->user_id)) {
...
//                            }
                ...
                'cancel' => function ($url, $model, $id) {
                    if ($model->status != \common\models\Offer::STATUS_CANCEL && $model->status != \common\models\Offer::STATUS_OK && $model->status != \common\models\Offer::STATUS_PENDING && $model->status != \common\models\Offer::STATUS_REJECT) {

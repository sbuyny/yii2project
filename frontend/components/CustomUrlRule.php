...
class CustomUrlRule extends Object implements UrlRuleInterface {
    ...
    public function parseRequest($manager, $request)
    {
        try
        {
            ...
            $route = UrlRule::getRouteBySlug($_slug);
        ...
        if (is_object($dbRoute) && $dbRoute->hasAttribute('redirect'))
        {
            if ($dbRoute->getAttribute('redirect'))
            {
                Yii::$app->response->redirect(
                        [$dbRoute->slug], $dbRoute->getAttribute('redirect_code')
                );
            }
        }
    ...
    private function getRouteFromCacheOrWriteCacheThenRead($_route, $_params)
    {
        ...
        if ($dbRoute == false)
        {
            $dbRoute = UrlRule::getRoute($_route, $_params);

            Yii::$app->cache->set(
                    $this->getCachedRoute($_route, $_params), $dbRoute
            );
        }
